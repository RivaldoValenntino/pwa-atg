...
import AbsenLemburCard from "../../components/AbsenLemburCard";
import { riwayatAbsenLemburQueryOptions } from "../../queries/absenLemburQueryFn";
import MonthPicker from "../../components/MonthPicker";

export const Route = createFileRoute("/_authenticated/absen-lembur")({
  component: RouteComponent,
  loader: async ({ context }) => {
    const period = new Date().toISOString().slice(0, 6); // Default to current year-month (YYYY-MM)

    await context.queryClient.prefetchQuery(
      riwayatAbsenLemburQueryOptions(period)
    );
  },
});
...
function RouteComponent() {
  ...
  const { data } = useSuspenseQuery(
    riwayatAbsenLemburQueryOptions(selectedPeriod)
  );
  ...
  return (
    <div className="font-poppins relative h-screen bg-gray-100">
      {/* Header Section */}
      <div className="bg-primary text-white rounded-b-2xl shadow-md p-6 h-52 relative overflow-hidden">
        {/* Back Arrow Section */}
        <div className="absolute top-4 left-4 flex items-center space-x-4">
          ...
          <p className="text-lg font-semibold">Absensi Lembur</p>
        </div>
      ...
      <div className="absolute bg-gray-100 w-full h-[70%] p-6 rounded-t-2xl -mt-16">
        ...
        <h2 className="flex sm:justify-center text-sm sm:text-xl font-bold my-4">
          Riwayat Absen Lembur Bulan Ini
        </h2>
